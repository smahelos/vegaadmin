@php
    // Explicitní nastavení jazyka pro tento view
...
<head>
    ...
    <style>
        ...
        .supplier-info h3, .client-info h3 {
            margin-top: 0;
            margin-bottom: 10px;
            font-size: 14px;
            text-transform: uppercase;
...
<body>
    ...
    <div class="parties clearfix">
        <div class="col-left">
            ...
            <div class="supplier-info">
                ...
                <p>{{ $supplier->zip ?? $invoice->zip ?? '' }} {{ $supplier->city ?? $invoice->city ?? '' }}</p>
                <p>{{ $supplier->country ?? $invoice->country ?? '' }}</p>
        ...
        <div class="col-right">
            ...
            <div class="client-info">
                ...
                <p>{{ $client->zip ?? $invoice->client_zip ?? '' }} {{ $client->city ?? $invoice->client_city ?? '' }}</p>
                <p>{{ $client->country ?? $invoice->client_country ?? '' }}</p>
        ...
        <div class="clearfix">
            <div class="col-left">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ \Carbon\Carbon::parse($invoice->tax_point_date)->format('d.m.Y') }}</span>
                </div>
                <div class="info-row">
                ...
                <div class="info-row">
                    <span class="info-label">{{ __('invoices.fields.due_date') }}:</span> 
                    <span class="info-value">{{ \Carbon\Carbon::parse($invoice->issue_date)->addDays($invoice->due_in)->format('d.m.Y') }}</span>
                </div>
            </div>
    ...
    <div class="payment-details">
        ...
        @php
            // Ošetření, že due_in může být string místo int
            $dueIn = isset($invoice->due_in) ? (int)$invoice->due_in : 14;
            $issueDate = isset($invoice->issue_date) ? \Carbon\Carbon::parse($invoice->issue_date) : \Carbon\Carbon::now();
            $dueDate = $issueDate->copy()->addDays($dueIn);
    ...
    <div class="section">
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td>{{ $invoice->invoice_text ?? __('invoices.defaults.invoice_text') }}</td>
                    <td class="text-right">{{ number_format($invoice->payment_amount, 2, ',', ' ') }} {{ $invoice->payment_currency }}</td>
                </tr>
            </tbody>
            <tfoot>
                <tr class="amount-total">
                    <th class="text-right">{{ __('invoices.fields.total') }}</th>
                    <td class="text-right"><strong>{{ number_format($invoice->payment_amount, 2, ',', ' ') }} {{ $invoice->payment_currency }}</strong></td>
                </tr>
            </tfoot>
        </table>
    </div>

    <!-- Platební informace a QR kód -->
        
        @php
            // Ošetření bankovních údajů pro různé typy objektů
    ...
    <div class="clearfix">
        <div class="@if(isset($hasQrCode) && $hasQrCode && !empty($qrCode))col-left @endif">
            <div class="payment-info">
                ...
                @if(!empty($bankName))
                    <div class="info-row">
                        <span class="info-label">{{ __('suppliers.fields.bank_name') }}:</span> <strong>{{ $bankName }}</strong>
                    </div>
                @elseif($invoice->supplier && !empty($invoice->supplier->bank_name))
                    <div class="info-row">
                        <span class="info-label">{{ __('suppliers.fields.bank_name') }}:</span> <strong>{{ $invoice->supplier->bank_name }}</strong>
                    </div>
                @endif
                ...
                @elseif($invoice->supplier && !empty($invoice->supplier->account_number) && !empty($invoice->supplier->bank_code))
                    <div class="info-row">
                        <span class="info-label">{{ __('suppliers.fields.account_number') }}:</span> 
                ...
                @elseif($invoice->supplier && !empty($invoice->supplier->iban))
                    <div class="info-row">
                        <span class="info-label">{{ __('suppliers.fields.iban') }}:</span> <strong>{{ $invoice->supplier->iban }}</strong>
                    </div>
                @endif
                ...
                @elseif($invoice->supplier && !empty($invoice->supplier->swift))
                    <div class="info-row">
                        <span class="info-label">{{ __('suppliers.fields.swift') }}:</span> <strong>{{ $invoice->supplier->swift }}</strong>
                    </div>
                @endif
                
                <div class="info-row">
                    <span class="info-label">{{ __('invoices.fields.amount') }}:</span> 
                    <strong>{{ number_format($invoice->payment_amount, 2, ',', ' ') }} {{ $invoice->payment_currency }}</strong>
                </div>
        ...
        @if(isset($hasQrCode) && $hasQrCode && !empty($qrCode))
            <div class="col-right">
                <div class="qr-code">
                    ...
                    <img src="{{ $qrCode }}" alt="QR kód pro platbu">
                </div>
            </div>
        @endif
    </div>
