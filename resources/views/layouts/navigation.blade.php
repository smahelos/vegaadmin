<nav x-data="{ open: false }" class="bg-white border-b border-gray-200">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @auth
                        <x-nav-link :href="route('frontend.dashboard', ['lang' => app()->getLocale()])" :active="request()->routeIs('frontend.dashboard')">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                @auth
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="route('frontend.profile.edit', ['lang' => app()->getLocale()])">
                                {{ __('general.navigation.profile') }}
                            </x-dropdown-link>

                            <!-- Authentication -->
                            <form method="POST" action="{{ route('logout', ['lang' => app()->getLocale()]) }}" class="min-w-50">
                                @csrf
                ...
                @else
                    <div class="space-x-4">
                        <a href="@localizedRoute('login')" class="text-sm text-gray-700 underline">{{ __('users.actions.login') }}</a>
                        <a href="@localizedRoute('register')" class="text-sm text-gray-700 underline">{{ __('users.actions.register') }}</a>
                    </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @auth
                <x-responsive-nav-link :href="route('frontend.dashboard')" :active="request()->routeIs('frontend.dashboard', ['lang' => app()->getLocale()])">
