...
<div class="mb-6 flex justify-between items-center">
    <h1 class="text-3xl text-amber-600">{{ __('clients.titles.create') }}</h1>
    <div class="space-x-2">
...
<form action="{{ route('frontend.client.store') }}" method="POST">
                            ...
                            <div class="mb-5">
                                <label for="is_default" class="flex items-center">
                                    ...
                                    <input type="checkbox" 
                                        ...
                                        >
                                    <span class="ml-2 text-base font-medium text-gray-700">{{ __('clients.fields.is_default') }}</span>
                                </label>
                                @if(isset($field['hint']) && $field['hint'] !== '')
                                    <p class="mt-2 text-xs text-gray-500">{{ $field['hint'] }}</p>
                                @endif
                    ...
                    @foreach($fields as $field)
                        ...
                        @endif
                            ...
                            @if ($field['name'] === 'country')
                                ...
                                <x-country-select 
                                    name="country"
                                    :selected="old('country', $userInfo['country'] ?? 'CZ')" 
                                    required="false"
    ...
    <div class="mt-6 flex justify-between">
        ...
        <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:text-white bg-emerald-500 hover:bg-emerald-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 cursor-pointer">
            <i class="fas fa-save mr-2"></i>{{ __('clients.actions.create') }}
        </button>
    </div>
