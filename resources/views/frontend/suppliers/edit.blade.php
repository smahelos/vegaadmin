...
<form id="invoice-edit-form"
    action="{{ route('frontend.supplier.update', ['id' => $supplier->id, 'locale' => app()->getLocale()]) }}"
                    ...
                    <div class="md:col-span-1 @if(!$errors->has('supplier_logo'))mb-5 @endif">
                        <div id="image-preview-container">
                            ...
                            <img id="current-image-preview"
                                src="{{ Storage::disk('public')->url($supplier->supplier_logo) }}"
                                alt="{{ $supplier->name }}" class="max-h-40 object-cover rounded-sm">
                            @else
                            <p id="no-image-message" class="text-gray-500">{{ __('suppliers.messages.no_image') }}</p>
                            <img id="current-image-preview"
