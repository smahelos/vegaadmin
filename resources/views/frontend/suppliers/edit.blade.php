...
<form action="{{ route('frontend.supplier.update', $supplier->id) }}" method="POST">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-6 gap-6">
                    ...
                    <div
                        ...
                        <input type="{{ $field['type'] }}" name="{{ $field['name'] }}" id="{{ $field['name'] }}"
                            value="{{ old($field['name'], $supplier->{$field['name']}) }}"
                ...
                <div class="mb-5">
                    ...
                    <textarea name="{{ $field['name'] }}" id="{{ $field['name'] }}" rows="4"
                        ...
                        @endif>{{ old($field['name'], $supplier->{$field['name']}) }}</textarea>
                    @if(isset($field['hint']) && $field['hint'] !== '')
                    <p class="mt-2 text-xs text-gray-500">{{ $field['hint'] }}</p>
                    @endif
                    @error($field['name'])
                ...
                <!-- Default supplier -->
                <div class="mb-5 is-default-{{ $supplier->is_default }}">
                    <label for="is_default" class="flex items-center">
                        <input type="hidden" name="is_default" value="0">
                        <input type="checkbox" name="is_default" id="is_default" value="1"
                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                            {{ old('is_default', $supplier->is_default) ? 'checked' : '' }}>
                        <span class="ml-2 text-base font-medium text-gray-700">{{ __('suppliers.fields.is_default')
                            }}</span>
                    </label>
                    @if(isset($field['hint']) && $field['hint'] !== '')
                    <p class="mt-2 text-xs text-gray-500">{{ $field['hint'] }}</p>
        ...
        <div class="bg-green-50 overflow-hidden shadow-sm rounded-lg border border-green-200">
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-10 gap-6">
                    @endif
                    <div
                        class="@if($field['name'] === 'city')md:col-span-5 @elseif($field['name'] === 'account_number')md:col-span-4 @elseif($field['name'] === 'zip' || $field['name'] === 'bank_code' || $field['name'] === 'bank_name')md:col-span-3 @elseif($field['name'] === 'country')md:col-span-2 @else md:col-span-5 @endif ">
                        @if ($field['name'] === 'country')
                        <!-- Country -->
                        <x-country-select name="country" :selected="old('country', $supplier->country ?? 'CZ')"
    ...
    <div class="mt-6 flex justify-between">
        ...
        <button type="submit"
            ...
            <i class="fas fa-save mr-2"></i>{{ __('suppliers.actions.save_changes') }}
        </button>
    </div>
