...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($driver === 'mysql') {
            ...
            DB::statement("
                ...
                SET issue_date_new = CASE
                    WHEN issue_date IS NULL OR issue_date = '' OR issue_date = '0000-00-00' OR issue_date = '0000-00-00 00:00:00' THEN NULL
                    WHEN LENGTH(issue_date) >= 10 AND SUBSTR(issue_date, 5, 1) = '-' AND SUBSTR(issue_date, 8, 1) = '-' THEN DATE(issue_date)
                    ELSE NULL
                END
            ");
        }
        ...
        if ($driver === 'mysql') {
            ...
            // For SQLite, just add foreign keys without checking (Laravel handles duplicates)
            Schema::table('invoices', function (Blueprint $table) {
                try {
                    $table->foreign('client_id')->references('id')->on('clients')->onDelete('set null');
                } catch (\Exception $e) {
        ...
        Schema::table('banks', function (Blueprint $table) use ($driver) {
            ...
            if ($driver === 'mysql') {
                ...
                $indexes = DB::select("PRAGMA index_list(banks)");
                $indexExists = false;
                foreach ($indexes as $index) {
                    if ($index->name === 'banks_code_unique') {
                        $indexExists = true;
                        break;
                    }
                }
        ...
        Schema::table('clients', function (Blueprint $table) use ($driver) {
            if ($driver === 'mysql') {
                ...
                $indexExists = false;
                foreach ($indexes as $index) {
                    if ($index->name === 'clients_ico_unique') {
                        $indexExists = true;
                        break;
                    }
                }
        ...
        Schema::table('suppliers', function (Blueprint $table) use ($driver) {
            if ($driver === 'mysql') {
                ...
                $indexExists = false;
                foreach ($indexes as $index) {
                    if ($index->name === 'suppliers_ico_unique') {
                        $indexExists = true;
                        break;
                    }
                }
    ...
    public function down(): void
    {
        ...
        if ($driver === 'mysql') {
            ...
            // For SQLite, just try to drop foreign keys
            Schema::table('invoices', function (Blueprint $table) {
                try {
                    $table->dropForeign(['payment_method_id']);
                } catch (\Exception $e) {
