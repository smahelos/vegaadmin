...
/**
 * Unit tests for RegistrationRequest - CRITICAL RULE: Only pure business logic, no Laravel dependencies
 * 
...
class RegistrationRequestTest extends TestCase
{
    ...
    {
        $requiredMethods = ['authorize', 'rules', 'messages'];

        foreach ($requiredMethods as $method) {
            $this->assertTrue(
                method_exists($this->request, $method),
                "Method {$method} does not exist in RegistrationRequest class"
            );
        }
    }
