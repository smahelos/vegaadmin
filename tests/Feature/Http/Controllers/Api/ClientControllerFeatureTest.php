...
class ClientControllerFeatureTest extends TestCase
{
    ...
    public function admin_can_get_any_client(): void
    {
        ...
        // Admin can fetch any client's record
     ...
     */
    #[Test]
    public function admin_endpoint_ownership_behavior(): void
    {
        // Admin can access foreign client -> 200
        $foreign = $this->actingAs($this->admin, 'backpack')
            ->getJson("/api/admin/client/{$this->userClient->id}");
        ...
        // Own client -> 200
        $own = $this->actingAs($this->admin, 'backpack')
            ->getJson("/api/admin/client/{$this->adminClient->id}");
        $own->assertStatus(200)
            ->assertJson([
                'id' => $this->adminClient->id,
                'user_id' => $this->admin->id,
            ]);
    }
    ...
    public function regular_endpoint_enforces_ownership_checks(): void
    {
        ...
        // Implementation returns 404 for not found / forbidden
        $response->assertStatus(404)
            ->assertJson([
                'error' => __('clients.messages.not_found'),
            ]);
