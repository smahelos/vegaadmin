...
use App\Models\Tax;
use App\Domain\Shared\Tax\Services\TaxService as TaxesService;
use App\Domain\Shared\Tax\Contracts\TaxServiceInterface as TaxesServiceInterface;
...
class TaxesServiceFeatureTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->service = app(TaxesService::class);
    ...
    public function get_all_taxes_uses_cache(): void
    {
        ...
        // Clear any existing cache via service (uses tags)
        $this->service->clearCategoriesCache();

        // Act - First call should cache the result
        $result1 = $this->service->getAllTaxes();
        
        // Delete the tax from database without firing observers (to avoid cache invalidation)
        Model::withoutEvents(function () use ($tax) {
            $tax->delete();
        });
        ...
        // Assert cached value is returned despite DB change
    ...
    {
        // Arrange
        Tax::factory()->create(['name' => 'Test Tax', 'slug' => 'test-tax']);

        // Act - First call to populate cache
        $initial = $this->service->getAllTaxes();
        $this->assertArrayHasKey('test-tax', $initial);

        // Before TTL: add new tax but expect cached result (no new key)
        $this->travel(4)->minutes();
        Model::withoutEvents(function () {
            Tax::factory()->create(['name' => 'New Tax', 'slug' => 'new-tax']);
        });
        $beforeExpire = $this->service->getAllTaxes();
        $this->assertEquals($initial, $beforeExpire);
        $this->assertArrayNotHasKey('new-tax', $beforeExpire);

        // After TTL: expect refreshed result to include new key
        $this->travel(2)->minutes(); // Total 6 minutes
        $afterExpire = $this->service->getAllTaxes();
        $this->assertArrayHasKey('new-tax', $afterExpire);
    }
    ...
    {
        // Arrange
        $t1 = Tax::factory()->create(['name' => 'Test Tax']);

        // Act - First call to populate cache
        $initial = $this->service->getAllTaxesForSelect();
        $this->assertArrayHasKey($t1->id, $initial);

        // Before TTL: add new tax but expect cached result (no new id)
        $this->travel(4)->minutes();
        $t2 = Model::withoutEvents(function () {
            return Tax::factory()->create(['name' => 'New Tax']);
        });
        $beforeExpire = $this->service->getAllTaxesForSelect();
        $this->assertEquals($initial, $beforeExpire);
        $this->assertArrayNotHasKey($t2->id, $beforeExpire);

        // After TTL: expect refreshed result to include new id
        $this->travel(2)->minutes(); // Total 6 minutes
        $afterExpire = $this->service->getAllTaxesForSelect();
        $this->assertArrayHasKey($t2->id, $afterExpire);
    }
    ...
    public function clear_categories_cache_removes_all_tax_caches(): void
    {
        ...
        $initialSlug = $this->service->getAllTaxes();
        $initialSelect = $this->service->getAllTaxesForSelect();

        // Create a new tax while within TTL, results should still reflect cached values
        $new = Model::withoutEvents(function () {
            return Tax::factory()->create(['name' => 'Another Tax', 'slug' => 'another-tax']);
        });
        $beforeClearSlug = $this->service->getAllTaxes();
        $beforeClearSelect = $this->service->getAllTaxesForSelect();
        $this->assertEquals($initialSlug, $beforeClearSlug);
        $this->assertEquals($initialSelect, $beforeClearSelect);

        // Act
        $this->service->clearCategoriesCache();

        // Assert
        $afterClearSlug = $this->service->getAllTaxes();
        $afterClearSelect = $this->service->getAllTaxesForSelect();
        $this->assertArrayHasKey('another-tax', $afterClearSlug);
        $this->assertArrayHasKey($new->id, $afterClearSelect);
    }
