...
class InvoiceObserverFeatureTest extends TestCase
{
    ...
    $invoice->save(); // persist change so model has latest state
    // Call observer updated method directly (in app it would be via model event)
    ...
    $invoice->save();
    // Call observer updated method (invoice_text unchanged)
    $this->observer->updated($invoice);
